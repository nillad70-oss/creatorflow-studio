...
export default function Scripts() {
  ...
  const handleGenerate = async () => {
    ...
    setError(""); setGenerating(true); setGeneratedScript(null)
    try {
      const response = await fetch("/api/generate-script", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          topic, niche: profile?.niche, audience: profile?.audience,
          tone: profile?.tone, platform: profile?.preferred_platform,
  ...
  const handleSave = async () => {
    ...
    const { data, error: saveError } = await supabase.from("scripts").insert({
      user_id: user.id, title: generatedScript.title || topic, topic,
      hook: generatedScript.hook, body: generatedScript.body, cta: generatedScript.cta,
      pacing: generatedScript.pacing, script_mode: scriptMode, platform: profile?.preferred_platform,
    }).select().single()
  ...
  const handleOpenInTeleprompter = () => {
    if (!generatedScript) return
    localStorage.setItem("teleprompter_script", `${generatedScript.hook}\n\n${generatedScript.body}\n\n${generatedScript.cta}`)
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CREATOR_AGENTS.map((agent) => (
                    <button key={agent.id} onClick={() => setSelectedAgent(agent.id)}
                      className={`px-4 py-3 rounded-xl text-left transition-all duration-200 ${selectedAgent === agent.id ? "bg-electric/20 border border-electric/50" : "glass hover:border-electric/20"}`}>
              ...
              <div className="mb-5">
                ...
                <textarea value={topic} onChange={(e) => { setTopic(e.target.value); setError("") }}
                  ...
                  rows={3} className="input-field w-full px-4 py-3 rounded-xl text-sm resize-none" />
              </div>
              <div className="mb-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {SCRIPT_MODES.map((mode) => (
                    <button key={mode.id} onClick={() => setScriptMode(mode.id)}
                      className={`px-3 py-2.5 rounded-xl text-center transition-all duration-200 ${scriptMode === mode.id ? "bg-electric/20 border border-electric/50" : "glass hover:border-electric/20"}`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {CONTENT_GOALS.map((goal) => (
                    <button key={goal.id} onClick={() => setContentGoal(goal.id)}
                      className={`px-3 py-2.5 rounded-xl text-left transition-all duration-200 ${contentGoal === goal.id ? "bg-electric/20 border border-electric/50" : "glass hover:border-electric/20"}`}>
              ...
              {profile?.niche && <p className="text-tertiary text-xs text-center mt-3">Generating for {profile.niche} · {profile.audience}</p>}
              {savedScripts.length > 0 && (
                <div className="mt-8">
                  <h3 className="text-secondary text-xs uppercase tracking-widest mb-3">Saved Scripts</h3>
                  <div className="space-y-2">
                    {savedScripts.map((script) => (
                      <div key={script.id} onClick={() => setGeneratedScript({ title: script.title, hook: script.hook, body: script.body, cta: script.cta, pacing: script.pacing })}
                ...
                <div className="page-enter">
                  ...
                  {generatedScript.hashtags && (
                    <div className="glass rounded-2xl p-5 mb-3">
                      ...
                      <p className="text-primary text-sm">{Array.isArray(generatedScript.hashtags) ? generatedScript.hashtags.join(" ") : generatedScript.hashtags}</p>
                    </div>
                  )}
