...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (data.session) {
      ...
      window.location.href = '/api/stripe/checkout-signup?priceId=' + (process.env.NEXT_PUBLIC_STRIPE_MONTHLY_PRICE_ID || '') + '&userId=' + data.user.id + '&email=' + encodeURIComponent(form.email) + '&userId=' + data.user.id + '&email=' + encodeURIComponent(form.email)
    } else if (data.user) {
